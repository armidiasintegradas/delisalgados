-- DELI SALGADOS — V3.8 ORDER CUSTOMER IDENTITY RPC
...
begin
  ...
  insert into public.orders (
    ...
    handoff_token_hash, handoff_token_expires_at, created_at, updated_at
  ) values (
    ...
    (p_order->>'total')::numeric(10,2), 'generated', 'pending',
    p_token_hash, p_token_expires_at, now(), now()
  );
  ...
  select jsonb_build_object(
    'id', o.id, 'public_code', o.public_code, 'customer_name', o.customer_name,
    'customer_phone', o.customer_phone, 'customer_email', o.customer_email,
    'customer_user_id', o.customer_user_id, 'desired_date', o.desired_date,
    'fulfillment_type', o.fulfillment_type, 'delivery_address', o.delivery_address,
    'customer_reference_point', o.customer_reference_point,
    'customer_note', o.customer_note, 'total', o.total, 'status', o.status,
    'whatsapp_status', o.whatsapp_status, 'created_at', o.created_at,
    'updated_at', o.updated_at,
    'items', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'id', oi.id, 'order_id', oi.order_id, 'product_id', oi.product_id,
        'variant_id', oi.variant_id, 'product_name_snapshot', oi.product_name_snapshot,
        'variant_name_snapshot', oi.variant_name_snapshot,
        'unit_label_snapshot', oi.unit_label_snapshot,
        'unit_price_snapshot', oi.unit_price_snapshot, 'quantity', oi.quantity,
        'subtotal', oi.subtotal, 'note', oi.note
...
revoke all on function public.create_deli_order_v34(jsonb,jsonb,text,timestamptz) from public, anon, authenticated;
